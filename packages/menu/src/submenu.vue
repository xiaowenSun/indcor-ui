<script>
  ...
  export default {
    ...
    computed: {
      ...
      menuTransitionName() {
        return this.rootMenu.collapse ? 'gpzx-zoom-in-left' : 'gpzx-zoom-in-top';
      },
      opened() {
      ...
      isFirstLevel() {
        ...
        while (parent && parent !== this.rootMenu) {
          if (['GpzxSubmenu', 'GpzxMenuItemGroup'].indexOf(parent.$options.componentName) > -1) {
            isFirstLevel = false;
            break;
          } else {
    ...
    methods: {
      ...
      handleClick() {
        ...
        this.dispatch('GpzxMenu', 'submenu-click', this);
      },
      handleMouseenter(event, showTimeout = this.showTimeout) {
        ...
        this.dispatch('GpzxSubmenu', 'mouse-enter-child');
      ...
      handleMouseleave(deepDispatch = false) {
        ...
        this.dispatch('GpzxSubmenu', 'mouse-leave-child');
        clearTimeout(this.timeout);
        this.timeout = setTimeout(() => {
        ...
        if (this.appendToBody && deepDispatch) {
          if (this.$parent.$options.name === 'GpzxSubmenu') {
    ...
    render(h) {
      ...
      const popupMenu = (
        <transition name={menuTransitionName}>
          <div
            ref="menu"
            v-show={opened}
            class={[`gpzx-menu--${mode}`, popperClass]}
            ...
            <ul
              role="menu"
              class={['gpzx-menu gpzx-menu--popup', `gpzx-menu--popup-${currentPlacement}`]}
      ...
      const inlineMenu = (
        <gpzx-collapse-transition>
          <ul
            role="menu"
            class="gpzx-menu gpzx-menu--inline"
            v-show={opened}
            style={{ backgroundColor: rootMenu.backgroundColor || '' }}>
            {$slots.default}
          </ul>
      ...
      return (
        ...
        >
          ...
          >
            {$slots.title}
            <i class={[ 'gpzx-submenu__icon-arrow', submenuTitleIcon ]}></i>
          </div>
