<template>
  <div class="gpzx-calendar">
    <div class="gpzx-calendar__header">
      <div class="gpzx-calendar__title">
        {{ i18nDate }}
      </div>
      <div
        class="gpzx-calendar__button-group"
...
import fecha from 'gpzx-ui/src/utils/date';
import GpzxButton from 'gpzx-ui/packages/button';
import GpzxButtonGroup from 'gpzx-ui/packages/button-group';
import DateTable from './date-table';
import { validateRangeInOneMonth } from 'gpzx-ui/src/utils/date-util';
  ...
  methods: {
    ...
    rangeValidator(date, isStart) {
      ...
      if (date.getDay() !== expected) {
        console.warn('[GpzxementCalendar]', message, 'Invalid range will be ignored.');
        return false;
      }
      return true;
    }
  ...
  computed: {
    ...
    validatedRange() {
      ...
      if (range.length === 2) {
        ...
        if (start > end) {
          console.warn('[GpzxementCalendar]end time should be greater than start time');
          return [];
        }
        ...
        if (!validateRangeInOneMonth(startDay, end)) {
          console.warn('[GpzxementCalendar]start time and end time interval must not exceed two months');
          return [];
        }
        // 第一个月的时间范围
        data.push([
