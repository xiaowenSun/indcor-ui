...
import { range as rangeArr, getFirstDayOfMonth, getPrevMonthLastDays, getMonthDays, getI18nSettings, validateRangeInOneMonth } from 'gpzx-ui/src/utils/date-util';
...
export default {
  ...
  methods: {
    ...
    cellRenderProxy({ text, type }) {
      let render = this.gpzxCalendar.$scopedSlots.dateCell;
  ...
  computed: {
    ...
    formatedToday() {
      return this.gpzxCalendar.formatedToday;
    },
        ...
        <tbody>
          {
            this.rows.map((row, index) => <tr
              class={{
                'gpzx-calendar-table__row': true,
                'gpzx-calendar-table__row--hide-border': index === 0 && this.hideHeader
              }}
              key={index}>
              {
