<template>
  <div
    ...
    :class="[
      'gpzx-cascader',
      realSize && `gpzx-cascader--${realSize}`,
    ...
    <div v-if="multiple" class="gpzx-cascader__tags">
      <gpzx-tag
      ...
      <input
        ...
        type="text"
        class="gpzx-cascader__search-input"
    ...
    <transition name="gpzx-zoom-in-top" @after-leave="handleDropdownLeave">
      <div
        v-show="dropDownVisible"
        ref="popper"
        :class="['gpzx-popper', 'gpzx-cascader__dropdown', popperClass]">
        <gpzx-cascader-panel
          ref="panel"
          v-show="!filtering"
          ...
          @close="toggleDropDownVisible(false)"></gpzx-cascader-panel>
        <gpzx-scrollbar
          ref="suggestionPanel"
          v-if="filterable"
          v-show="filtering"
          tag="ul"
          class="gpzx-cascader__suggestion-panel"
          view-class="gpzx-cascader__suggestion-list"
          ...
          <template v-if="suggestions.length">
            <li
              ...
              :key="item.uid"
              :class="[
                'gpzx-cascader__suggestion-item',
          ...
          <slot v-else name="empty">
            <li class="gpzx-cascader__empty-text">{{ t('el.cascader.noMatch') }}</li>
          </slot>
        </gpzx-scrollbar>
      </div>
    </transition>
...
import GpzxCascaderPanel from 'gpzx-ui/packages/cascader-panel';
import AriaUtils from 'gpzx-ui/src/utils/aria-utils';
import { t } from 'gpzx-ui/src/locale';
import { isEqual, isEmpty, kebabCase } from 'gpzx-ui/src/utils/util';
import { isUndefined, isFunction } from 'gpzx-ui/src/utils/types';
import { isDef } from 'gpzx-ui/src/utils/shared';
...
export default {
  ...
  computed: {
    realSize() {
      const _elFormItemSize = (this.gpzxFormItem || {}).elFormItemSize;
  ...
  methods: {
    ...
    focusFirstNode() {
      this.$nextTick(() => {
        ...
        if (filtering && suggestionPanel) {
          firstNode = suggestionPanel.$el.querySelector('.gpzx-cascader__suggestion-item');
        } else {
          const firstMenu = popper.querySelector('.gpzx-cascader-menu');
          firstNode = firstMenu.querySelector('.gpzx-cascader-node[tabindex="-1"]');
        }
    ...
    updateStyle() {
      ...
      const inputInner = $el.querySelector('.gpzx-input__inner');

      if (!inputInner) return;

      const tags = $el.querySelector('.gpzx-cascader__tags');
      let suggestionPanelEl = null;

      if (suggestionPanel && (suggestionPanelEl = suggestionPanel.$el)) {
        const suggestionList = suggestionPanelEl.querySelector('.gpzx-cascader__suggestion-list');
