<script>
  ...
  export default {
    ...
    watch: {
      visible(val) {
        this.broadcast('GpzxDropdownMenu', 'visible', val);
        this.$emit('visible-change', val);
      },
      focusing(val) {
        const selfDefine = this.$el.querySelector('.gpzx-dropdown-selfdefine');
        if (selfDefine) { // 自定义
          if (val) {
    ...
    methods: {
      ...
      initAria() {
        ...
        if (!this.splitButton) { // 自定义
          ...
          this.triggerElm.setAttribute('class', (this.triggerElm.getAttribute('class') || '') + ' gpzx-dropdown-selfdefine'); // 控制
        }
      },
      initEvent() {
    ...
    render(h) {
      ...
      let triggerElm = !splitButton
        ? this.$slots.default
        : (<gpzx-button-group>
          <gpzx-button type={type} size={dropdownSize} nativeOn-click={handleMainButtonClick}>
            {this.$slots.default}
          </gpzx-button>
      ...
      return (
        <div class="gpzx-dropdown" v-clickoutside={hide}>
          {triggerElm}
          {this.$slots.dropdown}
        </div>
      );
